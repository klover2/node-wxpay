...
class Pay extends IOrder {
    ...
    async unifiedorder(params) {
        ...
        case 'NATIVE': // pc端网站 模式二 (未测试)
            ...
            break;
        case 'MWEB': // 手机浏览器中支付  https://wx.tenpay.com/cgi-bin/mmpayweb-bin/checkmweb?prepay_id=wx20161110163838f231619da20804912345&package=1037687096&redirect_url=https%3A%2F%2Fwww.wechatpay.com.cn
    ...
    async refundquery(params) {
        if (!params.transaction_id && !params.out_trade_no && !params.out_refund_no && !params.refund_id) throw new Error('缺少参数transaction_id或out_trade_no或refund_id或out_refund_no!');
        const properties = [];
