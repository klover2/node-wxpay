...
class Withdraw extends IWithdraw {
    ...
    async sendredpack(params) {
        ...
        const properties = [ 'mch_billno', 'mch_id', 'wxappid', 'send_name', 're_openid', 'total_amount', 'total_num', 'wishing', 'client_ip', 'act_name', 'remark' ];
    ...
    async sendgroupredpack(params) {
        delete params['sign_type']; // 必须移除 不然会报密钥错误
        const properties = [ 'mch_billno', 'mch_id', 'wxappid', 'send_name', 're_openid', 'total_amount', 'total_num', 'amt_type', 'wishing', 'wishing', 'remark' ];
    ...
    async sendminiprogramhb(params) {
        delete params['sign_type']; // 必须移除 不然会报密钥错误
        const properties = [ 'mch_billno', 'mch_id', 'wxappid', 'send_name', 're_openid', 'total_amount', 'total_num', 'wishing', 'client_ip', 'act_name', 'remark', 'notify_way' ];
        ...
        if (result.return_code === 'SUCCESS' && result.result_code === 'SUCCESS' && result.package) {
            ...
            _data['signType'] = 'MD5';
            _data['nonceStr'] = chance.hash({ 'length': 20 }).toLowerCase();
